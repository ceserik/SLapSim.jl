...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		\node[leafbox] (hsl)     at (-3.0, -7.2) {ma27 ma57 ma97};
...
optimization as decision variables fixed to a value by constraints. This added them to the algebraic model. Then the solution of minimum lap time can be differentiated by these parameters. Results of sensitivity analysis are in Figure \ref{fig:sensitivity}.

\begin{figure}[H]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1\textwidth]{../sync/sensitivity.png}}
	\caption{Sensitivity of lap time to vehicle parameters.}
...
Ipopt has built-in Hessian approximation strategy, in benchmark it proved to be inferior to automatic differentiation. Figure \ref{fig:nlp_flowchart_chosen} shows which solving methods were implemented and tested.
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  >={Latex[length=2mm]},
...
\end{tikzpicture}
\caption{NLP solution flow with the methods implemented}
...
\end{tikzpicture}
\caption{Mesh refinement layer with the strategies implemented}
\label{fig:mesh_refinement_chosen}
\end{figure}

I have implemented p and h methods for mesh refinement. Error calculation is implemented by comparing the result of interpolated value at the end of segment and the result of simulation using Rodas4() solver. To calculate the error at the end of each segment, the interpolated controls and initial states are plugged into Rodas4 IVP solver. Then this result is compared to the states from transcription method. If error between states in absolute value is larger than user-defined threshold, the segment is halved. This is one of the simpler estimators of discretization error.
...
\end{tikzpicture}
\caption{Transcription layer with the implemented methods}
...
%\section{Actual constraints, example constraints like cannot leave road}
