...
\node[branch, below left=1cm and 2cm of nlp] (sqp) {SQP};
...
\end{tikzpicture}
\caption{NLP solution flow with the methods used in this thesis highlighted in green: interior point with barrier reformulation (IPOPT, MadNLP), automatic differentiation provided by JuMP, and the quasi-Newton (L-BFGS) Hessian approximation used in IPOPT's \texttt{limited-memory} mode.}
...
\end{tikzpicture}
\caption{Mesh refinement layer with the strategies implemented in this thesis highlighted in green: $p$-refinement (raising the polynomial degree on fixed segments) and $h$-refinement (subdividing segments at fixed degree).}
...
\end{tikzpicture}
\caption{Transcription layer with the methods used in this thesis highlighted in green: Legendre basis with all three Gauss-type quadrature schemes (LG, LGR, LGL), where LG and LGR are implemented in differential form and LGL in integral form.}
