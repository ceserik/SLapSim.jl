...
Implementation of specific models is conver in part III Implementation.
...
\begin{align}
    ...
    \label{QSS_model}
\end{align}
Where $\rho$ is air density $C_L$ is lift coefficient $v_x$ is longitudinal speed, $m$ is mass, $c$ is curvature of track in current point, $P_{max}$ is maximum power of motor, $\mu$ is friction coefficient, $C_D$ is drag coefficient.

This model has only one state $v_x$ which makes it possible to use specialized solvers. Variations of this model, where for example $P_{max}$ is a lookup table, the grip does not scale linearly with downforce etc. are commonly used in motor-sport.



\subsection{Single track}
A more complex model and useful model is single-track. In this model the suspension effects are neglected. Singletrack is still considered to be simplified model and is used most commonly when computation limits are reached.
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Twin-track model. Velocity flows from CoG through wheel pivots to tires; tire forces flow back. Motors deliver torque through gearboxes. Aerodynamics acts on the chassis; suspension exchanges loads and displacements with each wheel pivot. The chassis state $\symbf{x}$ is exported below the CoG.}
	\label{fig:twintrack}
\end{figure}
