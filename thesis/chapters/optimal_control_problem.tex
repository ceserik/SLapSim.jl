...
Mesh refinement is required to asses the accuracy of transcription, as optimization result without a guarantee on accuracy is useless. Mesh refinement works by changing polynomial order and number and place of segments. In mesh refinement it is also important to remove segments that are unnecessary. Mesh refinement methods are divided into three categories.
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  >={Latex[length=2mm]},
