...
The objective of collocation method is to find a polynomial that approximates the solution of a boundary value problem along independent variable (most commonly time, in this thesis it is distance travelled along centerline). Collocation methods are a subset of implicit Runge Kutta methods.
The main distinction between Runge Kutta methods and collocation, is that the result of collocation is always polynomial, that does not hold for general Runge Kutta method.

Collocation methods are based on theory of orthogonal polynomials
...
\end{tikzpicture}
\caption{Transcription layer: choice of orthogonal polynomial basis (top row), the resulting collocation point schemes derived from the Legendre basis (middle row), and the two equivalent algebraic formulations available for each scheme --- integral and differential form~\cite{garg_overview_nodate} (bottom row). Vertical dots under the other bases indicate that they admit their own families of quadrature schemes, which are not expanded here.}
