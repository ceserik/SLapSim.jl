...
There are numerous commercial and free algorithms to solve this system of linear equation. For smaller
...
Linear solvers are capable of multi threading on CPU, which needs to be explicitly
turned on. 
...
motor force in thousands of newtons and heading angle in radians. Newton steps in IPOPT are theoretically scale invariant, but the rest of the algorithm is not.~\cite[p.~21]{wachter_implementation_2006}. With improper scaling, Lagrange multiplers in \ref{eq:Newton-Step} may become large when active contraints are nearly linearly dependant and this causes numerical difficulties. TOTO SOM CELKOM OPSIAL Z IPOPT PAPERU ALE NEVIEM AKO TO INAK NAPISAT VLSTNYIM SLOVAMI.
...
deeply nested nonlinear expressions. Example of this is that state derivatives of vehicle at each point, can be fully determied from only 7 variables:  accelerator pedal, brake pedal, steering angle, forward speed, lateral speed, yaw rate and heading. Using only these variables results in a deeply nested nonlinear expression in the constraints. Adding redundant decision variables like forces on all wheels can reduce the iterations needed and calculation time of derivatives. Counter intuitively this larger problem was faster to solve, this is elaborated on in~\cite{julia_discourse_ipopt_hessian,jump_issue_4024}
