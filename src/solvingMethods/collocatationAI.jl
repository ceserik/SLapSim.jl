...
function create_gauss_pseudospectral_metod(f, pol_order, variant, model, nControls, nStates, track)
    ...
    function create_dynamic_constraints(segments, initialization)
        ...
        # FIX: For Legendre, U only needs one row per collocation point (no boundary slots).
        # Radau/Lobatto are unchanged.
        if variant == "Legendre"
        ...
        # Create U variables (Legendre: segments*pol_order rows; others: same as before)
        for i = 1:size(U, 1)
            for j = 1:nControls
                U[i, j] = @variable(model)
            end
        ...
        x_start_idx = 1
        u_start_idx = 1  # FIX: separate counter so U never touches boundary slots
        ...
        for i = 1:segments
            ...
            # Initialise X at the left boundary
            init_boundary = initialization.states(seg_nodes[1])
            for k = 1:nStates
                set_start_value(X[x_start_idx, k], init_boundary[k])
            end

            # Initialise U at the left boundary only for variants that have a U slot there
            if variant != "Legendre"
            ...
            for j = 1:n_ode_rows
                ...
                x_idx  = x_start_idx + j
                # FIX: for Legendre use the dedicated u counter; other variants share the X index
    ...
    function create_gauss_interpolator(X_vals, U_vals, all_nodes, segment_edges; hold_controls=true)
        ...
        # X stride: number of rows in X per segment
        x_stride = variant == "Legendre" ? pol_order + 1 :
                   variant == "Lobatto"  ? pol_order - 1 : pol_order

        # FIX: U stride differs for Legendre (no boundary slots → pol_order rows per segment)
        ...
        # FIX: for Legendre, build the physical positions of U points once so ZOH lookup
        # is correct without ever indexing into the (non-existent) boundary slots.
        u_positions = if variant == "Legendre"
            # For segment i, the X-space start is (i-1)*x_stride + 1.
            # all_nodes layout: boundary at x_start, then pol_order collocation nodes.
